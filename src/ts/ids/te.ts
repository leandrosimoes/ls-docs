export default class TE {
    ...
    validate(teNumber: string): boolean {
        ...
        if (
            !teNumber ||
            teNumber.length !== 12 ||
            this.COMMOM.indexOf(teNumber) !== -1
        )
            return false
        ...
        total =
            parseInt(teNumber[8]) * 7 +
            parseInt(teNumber[9]) * 8 +
            parseInt(firstDigit.toString()) * 9
        total = parseInt(((total % 11) + '').split('.')[0])

        secondDigit = total === 10 ? 0 : total

        return (
            parseInt(teNumber[teNumber.length - 2]) === firstDigit &&
            parseInt(teNumber[teNumber.length - 1]) === secondDigit
        )
    }
}
