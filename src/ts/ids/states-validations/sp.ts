...
export default class SP {
    ...
    static validate(value: string): boolean {
        ...
        if (value.substr(0, 1) === 'P') {
            ...
            let rest = helpers
                .getModule(base, [10, 8, 7, 6, 5, 4, 3, 1])
                .toString()
            ...
            let firstRest = helpers
                .getModule(firstBase, [10, 8, 7, 6, 5, 4, 3, 1])
                .toString()
            let first = firstRest.length > 1 ? firstRest[1] : firstRest[0]

            let secondRest = helpers
                .getModule(
                    firstBase + first + secondBase,
                    helpers.arrayFromTo(2, 10)
                )
                .toString()
